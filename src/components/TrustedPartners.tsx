...
const TrustedPartners = () => {
  ...
  const CompanyCard = ({ icon }: { icon: React.ReactNode }) => (
  ...
  useEffect(() => {
    if (marqueeRef1.current) {
      const w1 = marqueeRef1.current.scrollWidth / 2;
      setWidth1(w1);
    }

    if (marqueeRef2.current) {
      const w2 = marqueeRef2.current.scrollWidth / 2;
      setWidth2(w2);

      // 🔥 Start second row from negative width
      x2.set(-w2);
    }
  }, []);
  ...
  useAnimationFrame((t, delta) => {
    const pxPerSecond = 30; // 🔥 Adjust speed here
    const moveBy = (pxPerSecond / 1000) * delta;
    ...
    // Row 2 → Right direction (mirrored perfectly)
    if (width2) {
  ...
  return (
    <section className="bg-[#f6f7f4] py-[80px] overflow-hidden">
          ...
          <div className="marquee-wrapper">
            ...
            >
              {[...companies, ...companies].map((company, idx) => (
                <CompanyCard key={idx} icon={company.icon} />
              ))}
            </motion.div>
          </div>
